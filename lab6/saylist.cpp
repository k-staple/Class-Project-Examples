...
/*collabed with Lauren for sort 
function and helped her with my way of checking if a added line is only 
blank
*/
...
void add(list<string>& vOfStrings) {
   ...
   int hasMeaning= 0; 		// check to see if all blank
   int i= 0;
   while (i<= strAdded.length()){
      if (!isspace(strAdded[i])){
         hasMeaning = 1;
         break;
      }
      i++;
   }
   if ((!strAdded.empty())&& (hasMeaning == 1)){
      vOfStrings.sort();           //strAdded being passed      
   }
...
/*
void sort(list<string>& lOfStrings)
{
   for (auto it= lOfStrings.begin(); it != lOfStrings.end(); it++){
   }
}
*/
